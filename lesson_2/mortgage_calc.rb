...
def get_monthly_payments(loan_amount, monthly_interest, loan_duration)
  return (loan_amount.to_f / loan_duration) if monthly_interest == 0
...
def get_loan_amount
  loop do
    ...
    amount = gets.chomp
    return amount.to_i if /^[1-9]\d*$/.match?(amount) # Any non-zero int
...
def get_loan_duration
  loop do
    ...
    duration = gets.chomp
    return duration.to_i if /^[1-9]\d*$/.match?(duration)
    puts "Not a valid number"
  end
end
...
def get_interest_rate
  loop do
    ...
    rate = gets.chomp
    # This regex matches digits followed by optional decimal point and more
    # optional digits. OR matches decimal point followed by digits.
    return BigDecimal(rate) if /^((\d+\.?\d*)|(\.\d+))$/.match?(rate)
    puts "Not a valid number"
  end
end
